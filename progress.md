...
**Focus:** Business Scenario – Product Sales Trend Analysis  

---

#### ✅ Task: Identify Products with Decreasing Monthly Sales (Last 3 Months)

**Objectives:**
- Prepare a product-wise monthly sales trend
- Detect products whose order volume consistently dropped over the last 3 months

---

#### 📊 Business Insights
- Several products (e.g., **Logitech Wireless Mouse**, **Avery Binder**) showed a **consistent drop** in order volume.
- These trends could indicate:
  - Reduced demand
  - Pricing or supply chain issues
  - Marketing or visibility drop
- Management can act proactively on declining performers

---

#### 💡 What I Learned
- Used `.dt.to_period('M')` to bucket dates by month
- Created a trend matrix using `.pivot()`
- Applied a custom row-level function to identify decreasing values
- Importance of filling missing month values with `.fillna(0)` before comparison

---

#### 🧠 Challenges / New Concepts
- Building dynamic logic for decreasing trend detection
- Formatting monthly columns chronologically
- Ensuring data consistency when pivoting on sparse months

---

#### 🧩 Code Snippet (Trend Check)
```python
# Detect products with 3-month decreasing trend
last_3 = sorted(pivot.columns)[-3:]

def is_decreasing(row):
    vals = row[last_3].values
    return vals[0] > vals[1] > vals[2]

decreasing_products = pivot[pivot.apply(is_decreasing, axis=1)]
